...
impl<T: UserEvent> Wry<T> {
  ...
  fn init(event_loop: EventLoop<Message<T>>) -> Result<Self> {
    ...
    let context = Context {
      ...
      // macOS: WebKit 系统自带,且 wry::webview_version() 在 macOS 26 上
      // 调用 NSBundle::bundleWithIdentifier("com.apple.WebKit") 会触发
      // CoreFoundation 段错误(见 tauri#13996),故直接置 true 跳过该调用。
      #[cfg(target_os = "macos")]
