# 青锁盾 · 青少年访问锁（grow-guard）

**青锁盾**是一款面向 macOS 的家长控制工具,帮助家长在孩子的电脑上管理「玩什么应用、能上哪些网站、什么时间能用」。它不依赖系统「屏幕使用时间」,以 root 守护进程 + 防火墙 + hosts 自建了一套可脚本化、可版本管理的管控能力,并针对「会 kill 进程 / 改配置 / 改 hosts」的技术型青少年做了防绕过设计。

> 适合给孩子建一个**标准(非管理员)账户**后使用。装好后一条命令即可配置限制,到点自动生效,进程被杀会自动重启,改配置文件会被识别为篡改。

![青锁盾桌面端界面](docs/images/app.png)
...
| **防卸载** | root LaunchDaemon(`KeepAlive` 自拉起,kill 立即重启)+ 配置 HMAC 签名(改文件被识别为篡改)+ 卸载需密码 |
...
| 途径 | 能否脚本自动配 | 说明 |
|------|:---:|------|
| Screen Time 屏幕使用时间 | 不能 | 无公开 CLI/API,数据在受 TCC 保护的 `knowledgeC.db`,只能在系统设置里手动点。系统里也没有 `screentime` 之类命令行工具 |
| 配置描述文件 `.mobileconfig` | 半自动 | 可脚本生成,但 macOS 13+ 起 `profiles` 命令**已不能静默安装**,必须在「系统设置 → 通用 → 设备管理」手动确认。能配网站黑白名单、禁用 App |
| **grow-guard(本工具)** | 能 | 一次 `sudo` 安装后完全命令行控制 |
